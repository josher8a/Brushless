...
describe('ExpressionAttributes', () => {
    describe('#addName', () => {
        ...
        it('should allow the addition of list index dereferences', () => {
            expect(Attribute.Path.toString(Attribute.Path.fromString('foo[2]'))).toEqual('#foo[2]');
        });

        it('should allow the addition of nested attributes', () => {
            expect(Attribute.Path.toString(Attribute.Path.fromString('foo.bar'))).toEqual('#foo.#bar');
        });

        it(
            'should allow the nesting of complex attributes to an arbitrary depth',
            () => {
                expect(Attribute.Path.toString(Attribute.Path.fromString('foo.bar[3].baz[4].quux.snap.crackle.pop[2][1][0]'))).toEqual(
